...
function resolveBaseUri() {
  ...
  if (env) {
    return vscode.Uri.file(env);
  }
  return vscode.Uri.file(path.resolve(process.cwd()));
}
var IndexService = class {
  files = /* @__PURE__ */ new Map();
  tagToFiles = /* @__PURE__ */ new Map();
  activeFilter = /* @__PURE__ */ new Set();
  maxNodes = vscode.workspace.getConfiguration().get("km.graph.maxNodes", 500);
  _onIndexUpdated = new vscode.EventEmitter();
  onIndexUpdated = this._onIndexUpdated.event;
  getBaseUri() {
  ...
  linkFileToTags(key, tags) {
    for (const t of tags) {
      if (!this.tagToFiles.has(t)) {
        this.tagToFiles.set(t, /* @__PURE__ */ new Set());
      }
      this.tagToFiles.get(t).add(key);
    }
  }
  unlinkFileFromTags(key, tags) {
    for (const t of tags) {
  ...
  async indexFile(uri, replace = false) {
    try {
      const buf = await vscode.workspace.fs.readFile(uri);
      const text = Buffer.from(buf).toString("utf-8");
      const fm = (0, import_gray_matter.default)(text);
  ...
  toGraph() {
    ...
    let nextId = 0;
    const pushNode = (id, label, kind) => {
      if (nodeId.has(id) || nodes.length >= this.maxNodes) {
        return;
      }
...
// src/views/tagsProvider.ts
var vscode2 = __toESM(require("vscode"));
var TagsProvider = class {
  constructor(index) {
    this.index = index;
  }
  ...
  getTreeItem(tag) {
    ...
    const item = new vscode2.TreeItem(active ? `\u25CF ${tag}` : tag);
    ...
    item.collapsibleState = vscode2.TreeItemCollapsibleState.None;
    return item;
  }
  async getChildren() {
...
var FilesProvider = class {
  ...
  getTreeItem(node) {
    if (isDir(node)) {
      const item2 = new vscode3.TreeItem(node.name, vscode3.TreeItemCollapsibleState.Collapsed);
      item2.resourceUri = node.uri;
      item2.contextValue = "dir";
      item2.description = `${node.children.size} items`;
      return item2;
    }
    const f = node.entry;
    const label = path2.basename(f.uri.fsPath);
    const item = new vscode3.TreeItem(label, vscode3.TreeItemCollapsibleState.None);
    item.command = { command: "vscode.open", title: "Open", arguments: [f.uri] };
    item.resourceUri = f.uri;
    item.contextValue = "file";
    item.description = f.tags.join(", ");
    item.tooltip = vscode3.workspace.asRelativePath(f.uri, false);
    return item;
  }
  ...
  buildTree(files) {
    const wsRoot = vscode3.workspace.workspaceFolders[0].uri;
    const baseSetting = vscode3.workspace.getConfiguration("notes").get("defaultNotesDir", "");
    const baseUri = baseSetting ? vscode3.Uri.joinPath(wsRoot, baseSetting) : wsRoot;
    const root = {
      type: "dir",
      name: baseSetting ? stripTrailingSlash(baseSetting) : vscode3.workspace.workspaceFolders[0].name,
      uri: baseUri,
  ...
  insertPath(node, parts, file) {
    ...
    const nextUri = vscode3.Uri.joinPath(node.uri, head);
    let child = node.children.get(head);
    if (!child || !isDir(child)) {
      child = { type: "dir", name: head, uri: nextUri, children: /* @__PURE__ */ new Map() };
      node.children.set(head, child);
    }
    this.insertPath(child, tail, file);
  }
...
function sortNodes(a, b) {
  ...
  const an = a.type === "dir" ? a.name : path2.basename(a.entry.uri.fsPath);
  const bn = b.type === "dir" ? b.name : path2.basename(b.entry.uri.fsPath);
  return an.localeCompare(bn, void 0, { sensitivity: "base" });
}

// src/views/mindmapsProvider.ts
var vscode4 = __toESM(require("vscode"));
var path3 = __toESM(require("path"));
var MindmapsProvider = class {
  constructor(workspace8) {
    this.workspace = workspace8;
  }
  ...
  async getChildren() {
    ...
    const files = await vscode4.workspace.findFiles("**/*.mindmap.json");
...
var MindmapItem = class extends vscode4.TreeItem {
  constructor(uri) {
    super(path3.basename(uri.fsPath), vscode4.TreeItemCollapsibleState.None);
    this.uri = uri;
    this.resourceUri = uri;
    this.description = vscode4.workspace.asRelativePath(uri, false);
...
// src/views/TemplatesProvider.ts
var vscode5 = __toESM(require("vscode"));
var path4 = __toESM(require("path"));
var import_fs = require("fs");
var TemplatesProvider = class {
  ...
  async getChildren() {
    try {
      ...
      const files = await import_fs.promises.readdir(dir.fsPath).catch(() => []);
      return files.filter((f) => !f.startsWith(".")).map((f) => new TemplateLeaf({ uri: vscode5.Uri.joinPath(dir, f), name: f }));
    } catch {
      return [];
    }
  }
  async ensureFolder() {
    const folder = vscode5.workspace.getConfiguration().get("km.templates.workspaceFolder", "templates");
    const root = vscode5.workspace.workspaceFolders?.[0]?.uri;
    if (!root) {
      return vscode5.Uri.file("");
    }
    const target = root.with({
      path: path4.posix.join(root.path, folder.replace(/\\/g, "/"))
    });
    await vscode5.workspace.fs.createDirectory(target);
    return target;
  }
};
var TemplateLeaf = class extends vscode5.TreeItem {
  constructor(entry) {
    super(entry.name, vscode5.TreeItemCollapsibleState.None);
    this.entry = entry;
...
async function renderGraphHtml(ctx, panel, initial) {
  const webview = panel.webview;
  const mediaRoot = vscode6.Uri.joinPath(ctx.extensionUri, "media", "graph");
  const indexHtmlUri = vscode6.Uri.joinPath(mediaRoot, "index.html");
  const scriptUri = webview.asWebviewUri(vscode6.Uri.joinPath(mediaRoot, "main.js"));
  const styleUri = webview.asWebviewUri(vscode6.Uri.joinPath(mediaRoot, "styles.css"));
  ...
  let html = await (0, import_promises.readFile)(indexHtmlUri.fsPath, "utf8");
  html = html.replace(/{{CSP}}/g, csp).replace(/{{STYLE_URI}}/g, String(styleUri)).replace(/{{SCRIPT_URI}}/g, String(scriptUri)).replace(/{{NONCE}}/g, nonce).replace(/{{BOOTSTRAP}}/g, `<script nonce="${nonce}">window.initial=${JSON.stringify(initial)};</script>`);
  return html;
}
function openGraphWebview(ctx, index) {
  const column = vscode6.workspace.getConfiguration().get("km.twoPane", true) ? vscode6.ViewColumn.Beside : vscode6.ViewColumn.Active;
  const panel = vscode6.window.createWebviewPanel("notesGraph", "Notes Graph", column, {
    enableScripts: true,
    retainContextWhenHidden: true
  });
  const initialGraph = index.toGraph();
  renderGraphHtml(ctx, panel, { graph: initialGraph }).then((html) => {
  ...
  panel.webview.onDidReceiveMessage(async (msg) => {
    ...
    if (msg.type === "open" && typeof msg.key === "string" && !msg.key.startsWith("tag:")) {
      const uri = vscode6.Uri.parse(msg.key);
      vscode6.window.showTextDocument(uri, { viewColumn: vscode6.ViewColumn.One });
    }
    if (msg.type === "toggleTag" && typeof msg.tag === "string") {
      index.toggleTagFilter(msg.tag);
    }
  });
}

// src/webviews/mindmapWebview.ts
...
async function renderMindmapHtml(ctx, panel, initial) {
  const webview = panel.webview;
  const mediaRoot = vscode7.Uri.joinPath(ctx.extensionUri, "media", "mindmap");
  const indexHtmlUri = vscode7.Uri.joinPath(mediaRoot, "index.html");
  const scriptUri = webview.asWebviewUri(vscode7.Uri.joinPath(mediaRoot, "main.js"));
  const styleUri = webview.asWebviewUri(vscode7.Uri.joinPath(mediaRoot, "styles.css"));
  ...
  let html = await (0, import_promises2.readFile)(indexHtmlUri.fsPath, "utf8");
  html = html.replace(/{{CSP}}/g, csp).replace(/{{STYLE_URI}}/g, String(styleUri)).replace(/{{SCRIPT_URI}}/g, String(scriptUri)).replace(/{{NONCE}}/g, nonce).replace(
    /{{BOOTSTRAP}}/g,
    `<script nonce="${nonce}">window.initial=${JSON.stringify(initial)};</script>`
  );
  return html;
}
async function openMindmapWebview(ctx, uri) {
  ...
  migrateLinks(doc);
  const column = vscode7.workspace.getConfiguration().get("km.twoPane", true) ? vscode7.ViewColumn.Beside : vscode7.ViewColumn.Active;
  const ws = vscode7.workspace.workspaceFolders?.[0]?.uri;
  const panel = vscode7.window.createWebviewPanel(
    "notesMindmap",
    `Mindmap: ${vscode7.workspace.asRelativePath(target)}`,
    column,
    {
      enableScripts: true,
      retainContextWhenHidden: true
  ...
  function toWebSrc(p) {
    ...
    const u = vscode7.Uri.joinPath(ws, p);
    return panel.webview.asWebviewUri(u).toString();
  }
  const imgMap = Object.fromEntries(
    (doc.nodes || []).filter((n) => n.image?.path).map((n) => [n.image.path, toWebSrc(n.image.path)])
  );
  panel.webview.html = await renderMindmapHtml(ctx, panel, {
    doc,
    relPath: vscode7.workspace.asRelativePath(target),
    imgMap
  });
  panel.webview.onDidReceiveMessage(async (msg) => {
    if (!msg || typeof msg !== "object") {
    ...
    if (msg.type === "saveMindmap") {
      ...
      vscode7.window.setStatusBarMessage("Mindmap saved", 1500);
    }
    if (msg.type === "openFile" && typeof msg.path === "string") {
      if (!ws) {
        return;
      }
      const md = vscode7.Uri.joinPath(ws, msg.path);
      try {
        await vscode7.window.showTextDocument(md, { viewColumn: vscode7.ViewColumn.One });
      } catch {
        vscode7.window.showWarningMessage(`File not found: ${msg.path}`);
      }
    }
    if (msg.type === "pickMarkdown") {
      ...
      const rel = picked?.[0] ? vscode7.workspace.asRelativePath(picked[0]) : null;
    ...
    if (msg.type === "pickImage" && typeof msg.for === "string") {
      ...
      if (!picked?.[0]) {
        panel.webview.postMessage({ type: "pickedImage", for: msg.for });
        return;
      }
      if (!ws) {
        panel.webview.postMessage({ type: "pickedImage", for: msg.for });
      ...
      const assetsDir = vscode7.Uri.joinPath(ws, "mindmap-images");
      await ensureDir(assetsDir);
      const base = picked[0].path.split(/[/\\]/).pop();
      const t = vscode7.Uri.joinPath(assetsDir, base);
      await vscode7.workspace.fs.copy(picked[0], t, { overwrite: false });
      const rel = vscode7.workspace.asRelativePath(t);
      const src = toWebSrc(rel);
      panel.webview.postMessage({ type: "pickedImage", for: msg.for, path: rel, src });
    }
  });
}
async function pickOrCreateMindmap() {
  ...
  const ws = vscode7.workspace.workspaceFolders?.[0]?.uri;
  if (!ws) {
    vscode7.window.showWarningMessage("Open a folder to use mindmaps.");
    return;
  }
  const dir = vscode7.Uri.joinPath(ws, "mindmaps");
  await ensureDir(dir);
  if (choice.value === "new") {
    const basename4 = await vscode7.window.showInputBox({
      prompt: "Name for the mindmap file",
      value: `map-${(/* @__PURE__ */ new Date()).toISOString().slice(0, 10)}.mindmap.json`
    });
    if (!basename4) {
      return;
    }
    const file = vscode7.Uri.joinPath(dir, basename4);
...
async function readMindmapOrDefault(uri) {
  const buf = await vscode7.workspace.fs.readFile(uri);
...
async function writeMindmap(uri, doc) {
  const enc = new TextEncoder();
  await vscode7.workspace.fs.writeFile(uri, enc.encode(JSON.stringify(doc, null, 2)));
}
async function ensureDir(uri) {
  await vscode7.workspace.fs.createDirectory(uri);
}
function migrateLinks(doc) {
...
async function activate(ctx) {
  ...
  const mindmapsProvider = new MindmapsProvider(vscode8.workspace.workspaceFolders?.[0]);
  const templatesProvider = new TemplatesProvider();
  vscode8.window.createTreeView("km.tagsView", { treeDataProvider: tagsProvider });
  vscode8.window.createTreeView("km.filesView", { treeDataProvider: filesProvider });
  vscode8.window.createTreeView("km.mindmapsView", { treeDataProvider: mindmapsProvider });
  vscode8.window.createTreeView("km.templatesView", { treeDataProvider: templatesProvider });
  ...
  const watcher = vscode8.workspace.createFileSystemWatcher("**/*.md");
  ...
  ctx.subscriptions.push(
    vscode8.workspace.onDidRenameFiles(async (e) => {
  ...
  ctx.subscriptions.push(
    vscode8.workspace.onDidChangeWorkspaceFolders(async () => await indexService.buildIndex())
  );
  function toTemplateEntry(arg) {
    if (!arg) {
      return void 0;
    }
    const anyArg = arg;
    if (anyArg.uri instanceof vscode8.Uri && typeof anyArg.name === "string") {
      return anyArg;
    }
    if (anyArg.resourceUri instanceof vscode8.Uri) {
      const uri = anyArg.resourceUri;
      return { uri, name: path5.basename(uri.fsPath) };
    }
    if (arg instanceof vscode8.Uri) {
      return { uri: arg, name: path5.basename(arg.fsPath) };
    }
    return void 0;
  }
  function getUriFromArg(arg) {
    if (!arg) {
      return void 0;
    }
    const a = arg;
    if (a?.type === "file" && a.entry?.uri) {
      return a.entry.uri;
    }
    if (a?.type === "dir" && a.uri) {
      return a.uri;
    }
    if (a?.type === "mindmap" && a.uri) {
      return a.uri;
    }
    if (a?.contextValue === "mindmapItem" && a.uri) {
      return a.uri;
    }
    if (a?.resourceUri instanceof vscode8.Uri) {
      return a.resourceUri;
    }
    if (a?.uri instanceof vscode8.Uri) {
      return a.uri;
    }
    if (a instanceof vscode8.Uri) {
      return a;
    }
    return void 0;
  }
  async function confirmDelete(kind, uri) {
    const rel = vscode8.workspace.asRelativePath(uri, false);
  ...
  async function confirmDeleteWithString(label) {
    const ok = await vscode8.window.showWarningMessage(
      `Delete ${label}?`,
      { modal: true },
      "Delete"
  ...
  ctx.subscriptions.push(
    ...
    vscode8.commands.registerCommand("km.openMindmap", (uri) => {
      if (uri) {
        openMindmapWebview(ctx, uri);
      } else {
        return pickAndOpenMindmap();
      }
    }),
    vscode8.commands.registerCommand("km.openMindmapQuickPick", () => pickAndOpenMindmap()),
    vscode8.commands.registerCommand("km.newMindmap", () => openMindmapWebview(ctx)),
    vscode8.commands.registerCommand("km.toggleTag", async (tag) => {
      indexService.toggleTagFilter(tag);
      tagsProvider.refresh();
      filesProvider.refresh();
    }),
    vscode8.commands.registerCommand("km.clearFilter", () => {
      indexService.clearFilter();
      tagsProvider.refresh();
      filesProvider.refresh();
    }),
    vscode8.commands.registerCommand("km.refreshMindmaps", () => {
      mindmapsProvider.refresh();
      vscode8.window.showInformationMessage("Mindmaps list refreshed.");
    }),
    vscode8.commands.registerCommand("km.deleteMindmap", async (arg) => {
      const uri = getUriFromArg(arg);
      if (!uri) {
        return;
      }
      const rel = vscode8.workspace.asRelativePath(uri, false);
      if (!await confirmDeleteWithString(`mindmap "${rel}"`)) {
        return;
      }
      await vscode8.workspace.fs.delete(uri, { recursive: false, useTrash: true });
      mindmapsProvider.refresh();
    }),
    vscode8.commands.registerCommand("km.addFile", async () => {
      const root = await getWorkspaceRoot();
      if (!root) {
        return;
      }
      const rel = await vscode8.window.showInputBox({
      ...
      const target = root.with({ path: path5.posix.join(root.path, relPosix) });
      await vscode8.workspace.fs.createDirectory(vscode8.Uri.file(path5.dirname(target.fsPath)));
      await vscode8.workspace.fs.writeFile(target, Buffer.from("", "utf8"));
      await indexService.updateFile(target);
      filesProvider.refresh();
      tagsProvider.refresh();
      vscode8.window.showTextDocument(target);
    }),
    vscode8.commands.registerCommand("km.addFolder", async () => {
      const root = await getWorkspaceRoot();
      if (!root) {
        return;
      }
      ...
      const relPosix = toPosixRelative(rel);
      const targetDir = root.with({ path: path5.posix.join(root.path, relPosix) });
      await vscode8.workspace.fs.createDirectory(targetDir);
      await indexService.buildIndex();
      filesProvider.refresh();
      tagsProvider.refresh();
      vscode8.window.showInformationMessage(`Folder created: ${vscode8.workspace.asRelativePath(targetDir, false)}`);
    }),
    vscode8.commands.registerCommand("km.refreshFiles", async () => {
      await indexService.buildIndex();
      filesProvider.refresh();
      tagsProvider.refresh();
      vscode8.window.showInformationMessage("Files list refreshed.");
    }),
    vscode8.commands.registerCommand("km.deleteFile", async (arg) => {
      const uri = getUriFromArg(arg);
      if (!uri) {
        return;
      }
    ...
    vscode8.commands.registerCommand("km.deleteFolder", async (arg) => {
      const uri = getUriFromArg(arg);
      if (!uri) {
        return;
      }
      if (!await confirmDelete("folder", uri)) {
        return;
      }
      await vscode8.workspace.fs.delete(uri, { recursive: true, useTrash: true });
      await indexService.buildIndex();
      filesProvider.refresh();
      tagsProvider.refresh();
    }),
    vscode8.commands.registerCommand("km.refreshTemplates", () => {
      templatesProvider.refresh();
      vscode8.window.showInformationMessage("Templates list refreshed.");
    }),
    vscode8.commands.registerCommand("km.newEmptyTemplate", async () => {
      const dir = await templatesProvider.ensureFolder();
      const name = await vscode8.window.showInputBox({ prompt: "Template file name", value: "template.md" });
      if (!name) {
        return;
      }
      const target = vscode8.Uri.joinPath(dir, name);
      await vscode8.workspace.fs.writeFile(target, Buffer.from("# New Template\n", "utf8"));
      templatesProvider.refresh();
      vscode8.window.showTextDocument(target);
    }),
    vscode8.commands.registerCommand("km.saveActiveAsTemplate", async () => {
      ...
      const name = await vscode8.window.showInputBox({
        ...
        value: path5.basename(editor.document.fileName)
      });
      if (!name) {
        return;
      }
      const target = vscode8.Uri.joinPath(dir, name);
      await vscode8.workspace.fs.writeFile(target, Buffer.from(editor.document.getText(), "utf8"));
      templatesProvider.refresh();
    }),
    vscode8.commands.registerCommand("km.newFromTemplate", async (arg) => {
      const chosen = toTemplateEntry(arg) ?? await pickTemplate(templatesProvider);
      if (!chosen) {
        return;
      }
      const bytes = await vscode8.workspace.fs.readFile(chosen.uri);
      const raw = new TextDecoder("utf-8").decode(bytes);
      const content = await resolvePlaceholders(raw);
      const dest = await vscode8.window.showInputBox({
        prompt: "New file path (relative to workspace)",
      ...
      const target = root.with({ path: path5.posix.join(root.path, dest.replace(/\\/g, "/")) });
      await vscode8.workspace.fs.createDirectory(vscode8.Uri.file(path5.dirname(target.fsPath)));
      await vscode8.workspace.fs.writeFile(target, Buffer.from(content, "utf8"));
      vscode8.window.showTextDocument(target);
    }),
    vscode8.commands.registerCommand("km.rename", async (arg) => {
      const entry = toTemplateEntry(arg);
      if (!entry) {
        return;
      }
      const newName = await vscode8.window.showInputBox({ prompt: "New name", value: entry.name });
      if (!newName) {
        return;
      }
      const newUri = vscode8.Uri.joinPath(vscode8.Uri.file(path5.dirname(entry.uri.fsPath)), newName);
      await vscode8.workspace.fs.rename(entry.uri, newUri, { overwrite: false });
      templatesProvider.refresh();
    }),
    vscode8.commands.registerCommand("km.deleteTemplate", async (arg) => {
      const entry = toTemplateEntry(arg);
      if (!entry) {
        return;
      }
      ...
      if (ok === "Delete") {
        await vscode8.workspace.fs.delete(entry.uri, { recursive: false, useTrash: true });
        templatesProvider.refresh();
      }
    })
  );
  async function pickAndOpenMindmap() {
    const files = await vscode8.workspace.findFiles("**/*.mindmap.json");
    if (!files.length) {
      vscode8.window.showInformationMessage("No mindmaps found (*.mindmap.json).");
      return;
    }
    const picked = await vscode8.window.showQuickPick(
      files.map((f) => ({ label: vscode8.workspace.asRelativePath(f, false), description: f.fsPath, uri: f })),
      { placeHolder: "Open mindmap\u2026" }
    );
    if (picked?.uri) {
      openMindmapWebview(ctx, picked.uri);
    }
  }
  async function pickTemplate(provider) {
    const dir = await provider.ensureFolder();
    const files = await import_fs2.promises.readdir(dir.fsPath).catch(() => []);
    if (!files.length) {
      vscode8.window.showInformationMessage("No templates found.");
      return;
    }
    const qp = await vscode8.window.showQuickPick(files, { placeHolder: "Pick a template" });
    if (!qp) {
      return;
    }
    return { uri: vscode8.Uri.joinPath(dir, qp), name: qp };
  }
}
function suggestName(templateName) {
...
async function resolvePlaceholders(text) {
  ...
  text = await replaceAsync(
    text,
    inputRegex,
    async (_, prompt) => await vscode8.window.showInputBox({ prompt }) || ""
  );
  const pickRegex = /\$\{PICK:([^}]+)\}/g;
  text = await replaceAsync(text, pickRegex, async (_, opts) => {
    const options2 = opts.split("|").map((o) => o.trim());
    const pick = await vscode8.window.showQuickPick(options2, { placeHolder: "Pick a value" });
    return pick || "";
  });
  text = text.replace(/\$\{([A-Z]+)\}/g, (_, key) => table[key] ?? "");
  return text;
}
async function replaceAsync(str2, regex, asyncFn) {
  const promises = [];
  str2.replace(regex, (...args) => {
    promises.push(asyncFn(...args));
    return "";
  });
  const data = await Promise.all(promises);
  let i = 0;
  return str2.replace(regex, () => data[i++]);
}
